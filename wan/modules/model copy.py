...
class WanT2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x: torch.Tensor, context, context_lens):
        r"""
        Args:
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens):
        ...
        # compute query, key, value
        q = self.norm_q(self.q(x.float())).view(b, -1, n, d)
        k = self.norm_k(self.k(context.float())).view(b, -1, n, d)
        v = self.v(context.float()).view(b, -1, n, d)
        k_img = self.norm_k_img(self.k_img(context_img.float())).view(b, -1, n, d)
        v_img = self.v_img(context_img.float()).view(b, -1, n, d)
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            ...
            # with torch.amp.autocast(dtype=torch.float32):
                # x = x + y * e[5]
            x = gate_with_cp_grad_reduce(x.float(), e[5].float(), y.float()).contiguous()
